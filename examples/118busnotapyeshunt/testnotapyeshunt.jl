...
using MPCCases
include("/Users/jakeroth/git/OPF/src/OPF.jl") # comment out `module` part; this will ERROR @constraintref, but ok
...
bad_idx = findall(abs.(err_from_root(imported_operationpoint)) .> 1e-4)
[vecidx2busvar(i,imported_operationpoint) for i in bad_idx]
