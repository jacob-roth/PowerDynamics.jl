...
for branch_idx in 1:num_branches
    ...
    from_Bs = nobus_Bshunt ? 0.0 : first(buses[buses.bus_i .== from_bus].Bs)
    to_Bs = nobus_Bshunt ? 0.0 : first(buses[buses.bus_i .== to_bus].Bs)
...
open(casefile_path * "mpc_lowdamp_pgliblimits.y_shunt_nobus_Bshunt_arr", "w") do io
